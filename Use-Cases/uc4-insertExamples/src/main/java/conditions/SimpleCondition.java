...
public class SimpleCondition<E> extends Condition<E> {
	...
	public boolean evaluate(IPojo obj) {
		...
		if(obj instanceof Review)
			return evaluateReview((Review) obj);
		if(obj instanceof User)
			return evaluateUser((User) obj);
		return true;
	}
	...
	private boolean evaluateMovie(Movie obj) {
		...
		if(attr == MovieAttribute.dummy)
			objectValue = obj.getDummy();

		return operator.evaluate(objectValue, this.getValue());
	}
	private boolean evaluateReview(Review obj) {
		if(obj == null)
			return false;
		if(this.operator == null)
			return true;

		ReviewAttribute attr = (ReviewAttribute) this.attribute;
		Object objectValue = null;
		...
		if(attr == ReviewAttribute.note)
			objectValue = obj.getNote();

		return operator.evaluate(objectValue, this.getValue());
	}
	private boolean evaluateUser(User obj) {
		if(obj == null)
			return false;
		if(this.operator == null)
			return true;

		UserAttribute attr = (UserAttribute) this.attribute;
		Object objectValue = null;

		if(attr == UserAttribute.id)
			objectValue = obj.getId();
		if(attr == UserAttribute.username)
			objectValue = obj.getUsername();
