...
public class MovieActorServiceImpl extends dao.services.MovieActorService {
	...
	public Dataset<pojo.MovieActor> getMovieActorListInIMDB_MongoreviewColmovieactors(Condition<MovieAttribute> movie_condition, Condition<ActorAttribute> character_condition, MutableBoolean movie_refilter, MutableBoolean character_refilter){	
			List<String> bsons = new ArrayList<String>();
			String bson = null;
			bson = ActorServiceImpl.getBSONMatchQueryInReviewColFromMymongo(character_condition ,character_refilter);
			if(bson != null)
				bsons.add("{" + bson + "}");
			bson = MovieServiceImpl.getBSONMatchQueryInReviewColFromMymongo(movie_condition ,movie_refilter);
			if(bson != null)
				bsons.add("{" + bson + "}");
			...
			Dataset<MovieActor> res = dataset.flatMap((FlatMapFunction<Row, MovieActor>) r -> {
					...
					if(nestedRow != null && Arrays.asList(nestedRow.schema().fieldNames()).contains("movieid")) {
						if(nestedRow.getAs("movieid")==null)
							movieActor1.getMovie().setId(null);
						else{
							movieActor1.getMovie().setId((String)nestedRow.getAs("movieid"));
							toAdd1 = true;					
							}
					}
					// 	attribute Movie.primaryTitle for field title			
					...
					if(nestedRow != null && Arrays.asList(nestedRow.schema().fieldNames()).contains("title")) {
						if(nestedRow.getAs("title")==null)
							movieActor1.getMovie().setPrimaryTitle(null);
						else{
							movieActor1.getMovie().setPrimaryTitle((String)nestedRow.getAs("title"));
							toAdd1 = true;					
							}
					}
					...
					if(nestedRow != null && Arrays.asList(nestedRow.schema().fieldNames()).contains("avgrating")) {
						if(nestedRow.getAs("avgrating")==null)
							movieActor1.getMovie().setAverageRating(null);
						else{
							movieActor1.getMovie().setAverageRating((String)nestedRow.getAs("avgrating"));
							toAdd1 = true;					
							}
					}
					if(toAdd1) {
	...
	// method accessing the embedded object actors mapped to role movie
	public Dataset<pojo.MovieActor> getMovieActorListInIMDB_MongomovieColactors(Condition<MovieAttribute> movie_condition, Condition<ActorAttribute> character_condition, MutableBoolean movie_refilter, MutableBoolean character_refilter){	
			List<String> bsons = new ArrayList<String>();
			String bson = null;
			bson = ActorServiceImpl.getBSONMatchQueryInMovieColFromMymongo(character_condition ,character_refilter);
			if(bson != null)
				bsons.add("{" + bson + "}");
			bson = MovieServiceImpl.getBSONMatchQueryInMovieColFromMymongo(movie_condition ,movie_refilter);
			if(bson != null)
				bsons.add("{" + bson + "}");
			...
			Dataset<MovieActor> res = dataset.flatMap((FlatMapFunction<Row, MovieActor>) r -> {
					...
					if(nestedRow != null && Arrays.asList(nestedRow.schema().fieldNames()).contains("idmovie")) {
						if(nestedRow.getAs("idmovie")==null)
							...
							movieActor1.getMovie().setId((String)nestedRow.getAs("idmovie"));
							toAdd1 = true;					
							}
					}
					// 	attribute Movie.primaryTitle for field title			
						...
						for (int i2 = 0; i2 < array1.size(); i2++){
							...
							if(nestedRow != null && Arrays.asList(nestedRow.schema().fieldNames()).contains("actorid")) {
								if(nestedRow.getAs("actorid")==null)
									movieActor2.getCharacter().setId(null);
								else{
									movieActor2.getCharacter().setId((String)nestedRow.getAs("actorid"));
									toAdd2 = true;					
									}
							}
							...
							if(nestedRow != null && Arrays.asList(nestedRow.schema().fieldNames()).contains("name")) {
								if(nestedRow.getAs("name")==null)
									movieActor2.getCharacter().setFullName(null);
								else{
									movieActor2.getCharacter().setFullName((String)nestedRow.getAs("name"));
									toAdd2 = true;					
									}
							}
							if(toAdd2) {
