 ...
 *******************************************************************************/
package be.unamur.polystore.acceleo.main;
...
import be.unamur.polystore.acceleo.main.utils.Util;
...
public class Generate extends AbstractAcceleoGenerator {
	...
	public Generate(URI modelURI, File targetFolder, List<? extends Object> arguments) throws IOException {
		setArtifactAndGroupIdValues(modelURI);
	 ...
	 * @NOT generated
	 */
	private void setArtifactAndGroupIdValues(URI modelURI) {
		try {
			if (modelURI == null)
				return;
			...
			if (extension != null && !extension.isEmpty()) {
				int index = fileName.length() - (extension.length() + 1);
				if (index < 0)
					return;
				...
				if (m.find()) {
					...
					String groupId = m.group(3);
					Util.ARTIFACTID = artifactId;
					Util.GROUPID = groupId;
				}
			}
		} catch (Exception | Error e) {
			System.out.println(
					"The input file name does not respect pattern {artifactId}-{groupId}. Default values will used within the generated pom.xml");
		}
